...
def save_images(images, tag="phase1"):
    ...
    Strip calc to avoid nspins/eigenvalues metadata crash on read()."""
...
def _read_clean(path):
    ...
    with open(path) as f:
        lines = f.readlines()
    # Clean comment line (line 2): keep only standard keys
    if len(lines) > 1:
        comment = lines[1]
        # Extract key=value pairs, keep only standard ones
        parts = []
...
def load_images_if_exist(n_images, tag="phase1"):
    ...
    if all(f.exists() for f in files):
        imgs = [_read_clean(f) for f in files]
...
profile = AbacusProfile(
    # GPU mode: mpirun wrapper required to avoid MPI_Init_thread crash
    # on second subprocess call (CLAUDE.md rule #15: bare binary → zombie orted).
    # FIRE step 0 OK but step 1 fails with "MPI_Init_thread on NULL communicator".
...
def relax_endpoint(atoms, label, force_rerun=False):
    ...
    # Strip calc to avoid nspins/eigenvalues metadata in XYZ (ASE read() crash)
    def save_checkpoint():
        clean = atoms.copy()
        clean.calc = None
        write(str(ckpt_path), clean)
...
def run_two_phase_neb(endA, endB):
    ...
    # LBFGS for Phase 2: quasi-Newton, no momentum → no rebound/oscillation
    # Same principle as JDFTx electronic-minimize vs electronic-SCF (s98)
    # FIRE is great for Phase 1 (large forces), LBFGS for Phase 2 (near saddle)
    opt_p2 = LBFGS(
        neb_p2,
    ...
    print(f"  Running Phase 2 LBFGS (fmax<{FMAX_PHASE2}, max={MAX_STEPS_PHASE2})...",
          flush=True)
